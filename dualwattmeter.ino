...
#define PRINTHEADER "millis()\t\tlastPrintDisplay\twatthours\t\twattage" // a legend to the printed header

#define PRINTDISPLAYRATE 1000 // how many milliseconds between running printDisplay
#define LEDDISPLAYRATE 1000 // how many milliseconds between running updateDisplay

#define ENERGYPULSE     0.00118 // this many watt-hours have been used
...
unsigned long lastLEDDisplay = 0; // when's the last time we did updateDisplay
...
void updateWattage() {
  if (millis() - lastWattCalcTime > WATTCALCTIME) {
    wattage = (wattHours - lastWattCalcWattHours) * 3600000 / (millis() - lastWattCalcTime); // accuracy affected by millis() errors caused by *.show()
...
void updateDisplay() {
  if (millis() - lastLEDDisplay > LEDDISPLAYRATE) {
